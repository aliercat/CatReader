...
Windows 平台的本地小说阅读器，基于 Electron + React + TypeScript。
...
- **分页阅读**：CSS 多栏分页引擎；点击阅读区左/右 1/3 翻页，中间 1/3 呼出底部设置面板（字号、行距、页宽、1/2/3 栏、字体），变化后自动重排；阅读进度（章节 + 页码 + 显示设置）自动记忆。
- **阅读主题**：默认 / 护眼绿 / 羊皮纸 / 夜间四套阅读区主题，支持一键恢复默认；主题仅作用于阅读区，不改变应用界面外观。
