...
- **目录解析**：
  ...
  - epub：读取书内目录（OPF / spine / NCX / nav.xhtml），提取章节纯文本，正文插图以独立图片页展示。
- **书架**：封面网格展示（txt 生成首字渐变占位封面，epub 尝试提取封面图），支持书名/作者搜索、格式筛选（全部 / txt / epub）、排序（最近阅读 / 最近导入 / 书名）；已读书籍封面显示“已读 x%”续读徽标，点击续读。
- **分页阅读**：CSS 多栏分页引擎，翻页带轻量过渡动画；点击阅读区左 / 右 1/3 翻页，中间 1/3 呼出底部设置面板（字号、行距、页宽、1/2/3 栏、字体），调整后自动重排；页脚提供上一章 / 上一页 / 下一页 / 下一章快捷按钮与全书进度百分比。快捷键：`t` 目录、`s` 设置、`Ctrl+F` 本章搜索、`Ctrl+↑/↓` 切章、方向键/空格翻页。
- **目录侧边栏**：左侧滑出，支持按章节名实时搜索（长书快速定位），点击跳转。
- **阅读主题**：默认 / 护眼绿 / 羊皮纸 / 夜间四套阅读区主题，可一键恢复默认；主题只作用于阅读区。
- **界面外观**：书架、设置、目录等应用界面支持 跟随系统 / 亮色 / 深色 三态切换，选择会持久保存。
