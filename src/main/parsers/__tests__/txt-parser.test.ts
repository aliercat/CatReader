...
import { chapterContent, dedupeLeadingTitle, parseTxtToc, splitByLines } from '../txt-parser'
...
describe('dedupeLeadingTitle', () => {
  it('keeps a single leading title line (标题+正文样式)', () => {
    expect(dedupeLeadingTitle('第一章 开始\n正文第一段。\n继续。', '第一章 开始')).toBe(
      '第一章 开始\n正文第一段。\n继续。'
    )
    expect(dedupeLeadingTitle('第一章 开始', '第一章 开始')).toBe('第一章 开始')
  })

  it('removes extra consecutive title lines but keeps the first', () => {
    expect(dedupeLeadingTitle('第一章 开始\n第一章 开始\n正文第一段。', '第一章 开始')).toBe(
      '第一章 开始\n正文第一段。'
    )
    expect(
      dedupeLeadingTitle('第一章 开始\n 第一章 开始\n\n正文第一段。', '第一章 开始')
    ).toBe('第一章 开始\n\n正文第一段。')
  })

  it('keeps content when the first line differs from the title', () => {
    expect(dedupeLeadingTitle('正文第一段。\n第一章 开始\n再一段。', '第一章 开始')).toBe(
      '正文第一段。\n第一章 开始\n再一段。'
    )
    expect(dedupeLeadingTitle('开篇的话\n正文', '序章')).toBe('开篇的话\n正文')
  })

  it('handles empty content, empty title and CRLF line endings', () => {
    expect(dedupeLeadingTitle('', '第一章')).toBe('')
    expect(dedupeLeadingTitle('第一章 开始\n正文', '')).toBe('第一章 开始\n正文')
    expect(dedupeLeadingTitle('\r\n第一章 开始\r\n第一章 开始\r\n正文。', '第一章 开始')).toBe(
      '第一章 开始\r\n正文。'
    )
  })

  it('does not strip a longer first line that merely starts with the title', () => {
    expect(dedupeLeadingTitle('第一章 开始（上）\n正文', '第一章 开始')).toBe('第一章 开始（上）\n正文')
  })
})
